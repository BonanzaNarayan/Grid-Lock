...
export function GameChatInput({ roomId }) {
  ...
  async function send(payload) {
    ...
    try {
      await sendGameMessage({
        roomId,
        senderUid:   user.uid,
        displayName: profile.displayUsername,
        ...payload,
      });
    } finally {
      setSending(false);
  ...
  return (
    <div className="relative flex flex-col gap-0 border-t border-border bg-card shrink-0">
